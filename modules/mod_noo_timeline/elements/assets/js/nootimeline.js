...
!function ($) {
	...
	$.fn.nootimeline = function(options){
		return this.each(function(){
			...
			container.append('<div style="margin:0 0 10px 0"><button class="btn btn-small btn-success" id="nootimeline_add" type="button"><i class="icon-plus icon-white" title="New"></i>New</button></div>');
			container.append('<div class="ntm-items"></div>');
			var main_wap = $(".ntm-items");
			btn_add = $("#nootimeline_add");
			if($this.val() != ""){
			...
			function createTimeline(frame,title,description){
				...
				var html = '<div class="well well-small ntm-item"><button onclick="nootimelineRemove(this);" class="btn btn-small" id="nootimeline_remove" type="button"><i class="icon-remove icon-white" title="Remove"></i>Remove</button>'
...
function nootimelineRemove(element){
